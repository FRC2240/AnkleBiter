...
bool Vision::pose_loop()
{
  ...
  if(dev_check_l && dev_check_r)
    {
      pose_ret_val.trans_x
          = Vision::average(Vision::collect(&Data::trans_x, m_left_buffer),
                            Vision::collect(&Data::trans_x, m_right_buffer));
      pose_ret_val.trans_y
          = Vision::average(Vision::collect(&Data::trans_y, m_left_buffer),
                            Vision::collect(&Data::trans_y, m_right_buffer));
      pose_ret_val.rot_x
          = Vision::average(Vision::collect(&Data::rot_x, m_left_buffer),
                            Vision::collect(&Data::rot_x, m_right_buffer));
    ...
    {
      pose_ret_val.trans_x
          = Vision::average(Vision::collect(&Data::trans_x, m_right_buffer));
      pose_ret_val.trans_y
          = Vision::average(Vision::collect(&Data::trans_y, m_right_buffer));
      pose_ret_val.rot_x
          = Vision::average(Vision::collect(&Data::rot_x, m_right_buffer));
      Vision::update_pose(pose_ret_val);
      std::fill(m_right_buffer.begin(), m_right_buffer.end(), nonsense);
...
std::vector<double> Vision::collect(double Data::*f,
                                    std::vector<Data> const &v)
{
...
bool Vision::check_std_dev(std::vector<Data> buffer)
{
   ...
   **/
  if(Vision::find_good_frames(buffer) >= CONSTANTS::VISION::MIN_GOOD_FRAMES)
    {
      ...
      if(((x >= CONSTANTS::VISION::MIN_STD_DEV)
