...
public:
  ...
  /// @brief Flips the perspective of the drivetrain, so it rotates it's pose
  /// by 180 degrees.
  void flip();
  ...
  */
  ...
  // + Omega max speed * distance of module from center Or Robot Max Speed =
  // Max wheel speed - Omega max speed * distance from center Distance of
  // module from center is 1.294ft
  ...
  static constexpr units::acceleration::meters_per_second_squared_t
      TRAJ_MAX_ACCELERATION
      = TRAJ_MAX_SPEED / 0.5_s;
